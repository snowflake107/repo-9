...
class PublisherAgent:
    def __init__(self, output_dir: str, websocket=None, stream_output=None, lang='zh', headers=None):
...
"""
    ...
    async def run(self, research_state: dict):
        ...
        if self.websocket and self.stream_output:
            await self.stream_output("logs", "publishing",
                                     self.output.get_output('PUBLISHING'),
                                     self.websocket)
        else:
            print_agent_output(output=self.output.get_output(
                'PUBLISHING'), agent="PUBLISHER")
